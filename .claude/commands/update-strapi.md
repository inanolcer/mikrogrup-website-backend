---
description: Upgrade Strapi and all @strapi/* packages to a single target version
---
...
Upgrade Strapi (and all official `@strapi/*` packages) to a **single target version**. If the user did not name a version, ask which semver they want (e.g. `5.46.1`) before editing files.
...
2. Include at least: `@strapi/strapi`, `@strapi/plugin-documentation`, `@strapi/plugin-users-permissions`, `@strapi/provider-email-nodemailer`. Align any other `@strapi/*` entries if present (e.g. `@strapi/provider-email-amazon-ses`, `@strapi/provider-email-sendgrid`).
