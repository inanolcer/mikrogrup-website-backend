/**
 * printsgram-subscribe-form controller
 */
...
// Validation schema for printsgram subscribe form
const printsgramSubscribeFormSchema = yup.object().shape({
...
export default factories.createCoreController('api::printsgram-subscribe-form.printsgram-subscribe-form', ({ strapi }) => ({
  async create(ctx: any) {
    ...
    try {
      ...
      await printsgramSubscribeFormSchema.validate(data, { abortEarly: false, stripUnknown: false });
      ...
      if (extraFields.length > 0) {
        strapi.log.debug(`[Printsgram Subscribe Form] Received extra fields (ignored): ${extraFields.join(', ')}`);
      }
      ...
      if (data.createdAt) {
        strapi.log.debug(`[Printsgram Subscribe Form] Creating entry with custom createdAt: ${data.createdAt}`);
        
        // Use Document Service API directly to preserve createdAt
        result = await strapi.documents('api::printsgram-subscribe-form.printsgram-subscribe-form').create({
          data: filteredData,
        ...
        if (result && data.createdAt) {
          try {
            ...
            await strapi.db.query('api::printsgram-subscribe-form.printsgram-subscribe-form').update({
              where: { id: result.id },
              data: { 
            ...
            strapi.log.debug(`[Printsgram Subscribe Form] Updated timestamps for entry ${result.id}`);
          } catch (updateError) {
            strapi.log.warn(`[Printsgram Subscribe Form] Could not update custom timestamps: ${updateError.message}`);
          }
        }
      } else {
